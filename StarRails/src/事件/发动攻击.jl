...
@触发事件 begin
    ...
    for 状态 in 事件.来源.状态列表
        if 状态 isa 攻击强效 && 状态.强效标签 in 事件
            事件.威力 += 状态.数值
        end
    end
...
function 攻击目标!(事件::事件; 威力::Int, 来源 = 事件.来源, 目标 = 事件.目标, 性质::Vector{标签.性质} = 事件.性质列表)
    触发(发动攻击(事件.战场, 来源 = 来源, 目标 = 目标, 威力 = 威力, 性质 = 性质))
end
