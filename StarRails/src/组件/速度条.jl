...
function 插入单位!(速度条::速度条, 单位::角色, 下次行动的时间::Float64; 是额外回合::Bool = false, 是拉条::Bool = false)
...
function 拉条!(速度条::速度条, 单位::角色, 时间::Real)
    位置 = findfirst(节点 -> 节点.单位 == 单位, 速度条.节点)
    被拉条节点 = 速度条.节点[位置]
    被拉条节点.下次行动的时间 -= 时间
    if 被拉条节点.下次行动的时间 < 速度条.全局时间
        被拉条节点.下次行动的时间 = 速度条.全局时间
    end
    deleteat!(速度条.节点, 位置)
    插入单位!(速度条, 被拉条节点.单位, 被拉条节点.下次行动的时间, 是拉条 = true)
end
