...
function _根据期望函数选择目标(战场::战场, 来源::角色, 辅助::Bool, 期望函数::Function)
    当前最优的目标 = nothing
    当前最优的伤害::Float64 = -Inf
    可选的目标 = 辅助 ? 我方队伍(战场, 来源) : 敌方队伍(战场, 来源)
    if !辅助 && any(状态 -> 状态 isa 嘲讽, 来源.状态列表)
...
function 根据期望函数选择目标(战场::战场, 来源::角色, 期望函数::Function)
    return _根据期望函数选择目标(战场, 来源, false, 期望函数)
end

function 根据期望函数选择队友(战场::战场, 来源::角色, 期望函数::Function)
    return _根据期望函数选择目标(战场, 来源, true, 期望函数)
end
